...
- 普通用户应从 [v2.6.3 Release](https://github.com/an-X550/zhiji-desktop/releases/tag/v2.6.3) 下载 `Zhiji-Setup-v2.6.3.exe`。GitHub 自动生成的 `Source code (zip)` 是源码，不是安装程序；不要把源码压缩包直接当作应用运行。
- v2.6.3 已完成本地自动化、打包、安装器和隔离安装版 E2E 验收；本次没有安全可用的真实 AI 凭据，因此未宣称真实服务生成验收通过。配置你自己的 Key 后，才能验证实际服务可用性。
